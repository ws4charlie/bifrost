...
|------|-------------|-------------|
| **Admin** | Full access | Full access to all resources and operations, including log reveal |
| **Developer** | Technical access | CRUD access to technical resources, full inference access, and view access to logs and monitoring resources |
| **Viewer** | Read-only access | View access to resources and dashboards |
...
|----------|-------------|
| **Logs** | Request and response logs, including reversible redaction reveal when permitted |
...
`Logs:Reveal` controls access to the reveal endpoint for reversible guardrail redactions. A user with `Logs:View` can see the redacted log entry, but they also need `Logs:Reveal` to retrieve the placeholder-to-original mapping for that log.

Reveal applies only to Bifrost logs. It does not expose mappings to external observability connectors or object-storage exports. For redaction behavior, see [Guardrail Redaction](/enterprise/guardrails/redaction).

By default, Admin includes `Logs:Reveal`. Developer and Viewer include `Logs:View` but not `Logs:Reveal`.
