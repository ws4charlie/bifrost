...
Logs with reversible mappings expose `has_reversible_redaction` so the UI can show reveal controls to users with `Logs:Reveal`. If `disable_content_logging` is enabled, Bifrost skips request/response content and does not persist reveal data for that log.
