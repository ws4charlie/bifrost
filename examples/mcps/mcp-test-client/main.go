...
func authorize(cfg *config, h *transport.OAuthHandler) error {
	...
	if h.GetClientID() == "" {
		if err := h.RegisterClient(context.Background(), "bifrost-mcp-test-client"); err != nil {
			return fmt.Errorf("dynamic client registration: %w", err)
		}
	}

	authURL, err := h.GetAuthorizationURL(context.Background(), state, codeChallenge)
	if err != nil {
		return err
	}
	...
	if err := h.ProcessAuthorizationResponse(context.Background(), code, state, codeVerifier); err != nil {
